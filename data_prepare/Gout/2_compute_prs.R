...
library(bigreadr)
library(data.table)
folder = '/home/yinghaofu2/augmented_prs/data_prepare/clean_data/Gout/'
sumstats <- fread2('/home/yinghaofu2/augmented_prs/data_prepare/clean_data/Gout/GCST90475735_GRCh37.txt.gz', select = c(1,23,3,4,5,8))
names(sumstats) <- c("chr", "pos", "a1", "a0", "beta", "p")
sumstats$beta <- log(sumstats$beta)
sumstats <- subset(sumstats, p < 0.1)
sumstats$p[sumstats$p == 0] <- 1e-300
...
max(grid2$auc)  # 0.6464129
min(grid2$auc)  # 0.5526936
