...
library(bigreadr)
library(data.table)
folder = '/home/yinghaofu2/augmented_prs/data_prepare/clean_data/Hemorrhoids/'
sumstats <- fread2("/home/yinghaofu2/augmented_prs/data_prepare/clean_data/Hemorrhoids/GCST90086078_buildGRCh37.tsv")
colnames
sumstats <- fread2("/home/yinghaofu2/augmented_prs/data_prepare/clean_data/Hemorrhoids/GCST90086078_buildGRCh37.tsv", select = c(1,2,6,5,11,12))
colnames(sumstats)
...
library(bigreadr)
load(paste0(folder, 'feature.RData'))
table(y)
####     0      1 
####393082  10764
...
X <- X[eid %in% sub_eid]
saveRDS(y, file = paste0(folder, 'ukb_final_y.rds'))
saveRDS(X, file = paste0(folder, 'ukb_final_X.rds'))
...
rds <- bigsnpr::snp_readBGEN(
  ...
  ind_row = sub_id,
  backingfile = paste0(folder, 'imppgen_qc_unrelated_final'),
  ncores = NCORES)
)#149s


library(bigsnpr)
ukbb <- snp_attach(paste0(folder, 'imppgen_qc_unrelated_final.rds'))
G <- ukbb$genotypes
...
saveRDS(all_keep, file = paste0(folder, 'all_keep.rds'))
system.time(
  multi_PRS <- snp_grid_PRS(
    G, all_keep, betas = beta, lpS = lpval, ind.row = rows_along(G),
    n_thr_lpS = 50, 
    backingfile = paste0(folder, 'UKBB_scores'), ncores = NCORES)
) # 226s
saveRDS(multi_PRS, file = paste0(folder, 'multi_PRS.rds'))

all_keep <- readRDS(paste0(folder, 'all_keep.rds'))
multi_PRS <- readRDS(paste0(folder, 'multi_PRS.rds'))
...
max(grid2$auc)  # 0.5066098
min(grid2$auc)  # 0.494651
...
dim(multi_PRS_mat)
npySave(paste0(folder, 'multi_PRS.npy'), multi_PRS_mat)
y = readRDS(paste0(folder, 'ukb_final_y.rds'))
ids <- names(y)
write.table(
  ids,
  file = paste0(folder, "/ukb_final_ids.txt"),
  row.names = FALSE,
  col.names = FALSE,
  quote = FALSE
)
npySave(paste0(folder, 'ukb_final_y.npy'), as.matrix(y))
features <- readRDS(paste0(folder, 'ukb_final_X.rds'))
features <- features[,-1]
npySave(paste0(folder, 'ukb_final_features.npy'), as.matrix(features))
...
write.csv(semantic, paste0(folder, 'semantic.csv'), row.names = FALSE)
...
dim(covariates)  # 399713     23
dim(multi_PRS_mat)
dim(features)
dim(semantic)
length(y)
table(y)
npySave(paste0(folder, 'ukb_final_covariates.npy'), as.matrix(covariates[, -1]))
